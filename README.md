...
# fill name/contact in facts.yaml by hand, then ask your agent: "run the cv-gen facts interview"
...
files — edit them freely, a skill upgrade never touches them. `prompt.md`, `interview.md` and
`schema/` (`facts.schema.json`, `content.schema.json`) stay inside the skill and upgrade with
it, so pulling a new skill version tightens tailoring rules, interview rules or the schemas
without you re-applying local edits.
